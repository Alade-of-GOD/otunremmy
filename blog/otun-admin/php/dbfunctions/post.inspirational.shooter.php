<?php
	...
	$sqlpost = "SELECT * FROM `otun_posts` WHERE `category` = 'inspirational' ORDER BY `dateofpost` DESC LIMIT 0, 10";
    ...
    while($row = $result->fetch_assoc()){
        ...
        <article class="vce-post vce-lay-c post-191 post type-post status-publish format-standard has-post-thumbnail hentry category-food tag-chocolates tag-food-2 tag-magazine tag-sugar tag-sweet">
            ...
            <header class="entry-header">
                <span class="meta-category"><a href="otun-remmy-writes.cat.php?cat=poem&subcat='.$title.'" class="category-2">'.$title.'</a></span>
